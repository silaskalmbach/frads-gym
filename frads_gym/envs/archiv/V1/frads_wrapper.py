...
class EnergyPlusSimulation:
    ...
    """
    def __init__(self, output_dir=None, input_dir=None, run_annual=False, cleanup=True, run_period=None, treat_weather_as_actual=False, weather_files_path=None, number_of_timesteps_per_hour=1, epsetup_config=None, enable_radiance=True):
        """
...
def controller(state):
    ...
    for var_id, var_info in simulation.epsetup_config.items():
        if "calculate_wpi" in var_info:
            ...
            # Process lighting power calculations if configured
            if "lighting_power" in var_info:
                # Get the lighting power name from config
                lighting_power_name = var_info["lighting_power"]["name"]
                lux_threshold = var_info["lighting_power"]["lux"]
                power_density = var_info["lighting_power"]["power_density"]
                
                # Calculate lighting power based on the workplane illuminance
                lighting_power = (1 - min(obs_data[var_id] / lux_threshold, 1)) * power_density
                # obs_data[lighting_power_name] = lighting_power
                # Change the lighting power from W to lighting energy in J
                lighting_energy = lighting_power * 3600/simulation.number_of_timesteps_per_hour
                obs_data[lighting_power_name] = lighting_energy
    ...
    # 5. Process all calculate_energy_sum entries
    for var_id, var_info in simulation.epsetup_config.items():
        if "calculate_energy_sum" in var_info:
            ...
            for key in keys_to_sum:
                # Gets the value from obs_data. For 'lighting_energy_1',
                # this value is already energy (Joules) due to the calculation in step 3.
                value = obs_data.get(key)
                if value is not None:
                    try:
                        total_energy += float(value) # Ensure value is numeric
            ...
            # Stores the total energy sum (in Joules) in obs_data
    ...
    for var_id, var_info in simulation.epsetup_config.items():
        if "actuate_cfs_state" in var_info:
            ...
            action_key = var_id  # The key in action_values corresponding to this window
...
if __name__ == "__main__":
    ...
    epsim = EnergyPlusSimulation(output_dir="/home/simulation/Test", input_dir="/home/model/medium_office", run_annual=False, cleanup=True, run_period={"begin_year": 2000, "begin_month": 1, "begin_day": 1, "end_year": 2000, "end_month": 1, "end_day":3}, treat_weather_as_actual=False, weather_files_path=[weather_files["usa_ca_san_francisco"], weather_files["usa_nv_las_vegas"]], number_of_timesteps_per_hour=1)
    ...
    while True:
        ...
        action_values = random_action_values(observation_data)
