...
class EnergyPlusSimulation:
    ...
    def __init__(self, output_dir=None, config_file=None, run_annual=False, cleanup=True, run_period=None, treat_weather_as_actual=False, weather_files_path=None, number_of_timesteps_per_hour=1, enable_radiance=True):
        ...
        Args:
            ...
            config_file (str, optional): Path to the configuration file. If None, uses "epsetup_config.json" in the script directory.
        ...
        # Determine the input directory (where config and input files are located)
        if config_file is not None:
            # Get the directory containing the config file
            self.input_dir = os.path.dirname(os.path.abspath(config_file))
        else:
    ...
    def _simulation_config(self):
        ...
        print(f"Using run period: {first_run_period_key}")
        ...
        if not self.epmodel.run_period[first_run_period_key].begin_year or not self.epmodel.run_period[first_run_period_key].end_year:
            self.epmodel.run_period[first_run_period_key].begin_year = 2000
            self.epmodel.run_period[first_run_period_key].end_year = 2000
            
        # Calculate total timesteps
        start_date = datetime.datetime(
            self.epmodel.run_period[first_run_period_key].begin_year, 
            self.epmodel.run_period[first_run_period_key].begin_month, 
            self.epmodel.run_period[first_run_period_key].begin_day_of_month, 
            0, 0, 0
        )
        end_date = datetime.datetime(
            self.epmodel.run_period[first_run_period_key].end_year, 
            self.epmodel.run_period[first_run_period_key].end_month, 
            self.epmodel.run_period[first_run_period_key].end_day_of_month, 
            23, 59, 59
        )
    ...
    def reset(self):
        ...
        """
        # Thread-safe reset with complete cleanup
        with self.simulation_lock:
            # Shutdown existing simulation completely
            if hasattr(self, 'epsetup') and self.epsetup is not None:
                print("Shutting down existing simulation...")
                ...
                try:
                    ...
                    if hasattr(self.epsetup, 'enable_radiance') and self.epsetup.enable_radiance:
                        self._safe_cleanup_radiance()
                    ...
                    print(f"Warning during reset cleanup: {e}")
                finally:
                    self.epsetup = None
        ...
        # Initialize new EnergyPlus Simulation Setup
        try:
            print("Creating new EnergyPlus setup...")
            ...
            # Set context and register controller
            ...
            print("New simulation started successfully.")
            
        except Exception as e:
            print(f"Error creating new simulation: {e}")
            raise
    ...
    def shutdown(self):
        ...
        # Thread-safe shutdown with explicit resource cleanup
        with self.simulation_lock:
            # Signal all threads to terminate first
            self.shutdown_event.set()
            self.next_step_event.set()
            
            # Give threads time to react to shutdown signal
            time.sleep(0.3)
            
            # Wait for simulation to finish naturally
            self._wait_for_simulation_to_finish()

            # Clean up EnergyPlus and Radiance resources
            if hasattr(self, 'epsetup') and self.epsetup is not None:
                try:
                    print("Closing EnergyPlus setup...")
                    
                    # Cleanup Radiance resources first (if enabled)
                    if hasattr(self.epsetup, 'enable_radiance') and self.epsetup.enable_radiance:
                        self._safe_cleanup_radiance()
                    ...
                    # Ensure epsetup is set to None even if close() fails
                    self.epsetup = None
    ...
    def _safe_clear_queues(self):
        """Safely clear communication queues."""
        try:
            ...
            print(f"Warning clearing queues: {e}")

    def _safe_cleanup_radiance(self):
        """Safely cleanup Radiance resources to prevent memory corruption."""
        try:
            # Check for common Radiance attributes and clean them up
            if hasattr(self.epsetup, 'rworkflows') and self.epsetup.rworkflows:
                print("Cleaning up Radiance workflows...")
                for zone, workflow in list(self.epsetup.rworkflows.items()):
                    try:
                        # Clear matrices if they exist
                        if hasattr(workflow, 'matrices'):
                            workflow.matrices = None
                        
                        # Clear other workflow data
                        if hasattr(workflow, 'config'):
                            workflow.config = None
                            
                    except Exception as e:
                        print(f"Warning: Error cleaning workflow for {zone}: {e}")
                
                # Clear the workflows dict
                self.epsetup.rworkflows.clear()
                self.epsetup.rworkflows = None
            
            # Additional Radiance cleanup
            if hasattr(self.epsetup, 'rconfigs'):
                self.epsetup.rconfigs = None
            if hasattr(self.epsetup, 'rmodels'):
                self.epsetup.rmodels = None
                
        except Exception as e:
            print(f"Warning during Radiance cleanup: {e}")
    ...
    for var_id, var_info in simulation.epsetup_config.items():
        if "lighting_power_calculation" in var_info and "depends_on" in var_info:
            ...
            if depends_on_key in obs_data:
                ...
                power_density = var_info["lighting_power_calculation"]["power_density"]
